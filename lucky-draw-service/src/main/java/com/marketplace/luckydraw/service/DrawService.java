...
public class DrawService {
    ...
    public DrawResult draw(String campaignId, String sellerId, String correlationId) {
        ...
        if (campaign.status() == CampaignStatus.DRAWN) {
            var winner = entries.findEntryById(campaign.winnerEntryId()).orElseThrow(DomainException::notFound);
            return new DrawResult(winner, campaign.snapshotHash(), snapshots.selectedIndex(campaignId));
        }
        if (campaign.status() != CampaignStatus.ENDED) throw DomainException.notDrawable();

        var snapshot = snapshots.find(campaignId).orElseThrow(DomainException::notDrawable);
